...
function onWeaponProficiency(itemId, experience, perks, marketCategory)
	...
	-- Update the stats bar in real-time when proficiency data changes
	-- Only update if this is the currently equipped weapon (check both hands)
	local player = g_game.getLocalPlayer()
	if not player then return end
	
	-- Check left hand first (main weapon slot), then right hand
	local equippedWeapon = player:getInventoryItem(InventorySlotLeft)
	if not equippedWeapon or equippedWeapon:getId() ~= itemId then
		equippedWeapon = player:getInventoryItem(InventorySlotRight)
		if not equippedWeapon or equippedWeapon:getId() ~= itemId then
			return -- Not the equipped weapon, don't update the stats bar
		end
	end
	...
	if itemCache then
		...
		if thingType then
			local hasUnnusedPerk = false
			-- Check if there are available perks that haven't been selected
			-- Note: perks is a non-sequential table (keys are levels), so count manually
			local maxLevel = ProficiencyData:getCurrentLevelByExp(thingType, experience, false)
			local perkCount = 0
			if perks then
			...
			if perkCount < maxLevel then
				hasUnnusedPerk = true
			end
			modules.game_interface.StatsBar.onUpdateProficiencyData(itemCache, hasUnnusedPerk, thingType)
		end
	end
end
