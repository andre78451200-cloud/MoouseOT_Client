...
function inventoryController:onInit()
    ...
    connect(g_game, { onBlessingsChange = onBlessingsChange })
end
...
function inventoryController:onGameStart()
    ...
    inventoryController:registerEvents(LocalPlayer, {
        ...
        onBlessingsChange = function(player, blessings, oldBlessings)
            if not blessings then return end
            toggleAdventurerStyle(blessings > 0)
        end
    }):execute()
...
function inventoryController:onTerminate()
    disconnect(g_game, { onBlessingsChange = onBlessingsChange })
    if iconTopMenu then
        iconTopMenu:destroy()
        iconTopMenu = nil
    end
...
function onBlessingsChange(blessings, blessVisualState)
    local hasBlessing = blessVisualState >= 2 or blessings > 0
    toggleAdventurerStyle(hasBlessing)
    local ui = getInventoryUi()
    if not ui then return end
    local blessedButton = ui.blessings
    if not blessedButton then print('[BLESS] blessedButton is nil!') return end
