...
function bindKeys()
    ...
    g_keyboard.bindKeyPress('PageUp', function()
        if g_game.isInHouseMode() and lastClicked < g_clock.millis() then
            ...
            House.onUpdateEditingFloor(houseFloor)
    ...
    g_keyboard.bindKeyPress('PageDown', function()
        if g_game.isInHouseMode() and lastClicked < g_clock.millis() then
            local houseFloor = math.min(gameMapPanel.maxFloors or 10, g_game.getHouseFloor() + 1)
            House.onUpdateEditingFloor(houseFloor)
            lastClicked = g_clock.millis() + 200
        end
    end, gameRootPanel)
...
function onGameStart()
    ...
    -- Set LoadedPlayer data
    local player = g_game.getLocalPlayer()
    if player then
        local vocation = player:getVocation()
        local level = player:getLevel()
        local baseVocation = nil
        if player.getBaseVocation then
            baseVocation = player:getBaseVocation()
        end
        
        -- Fallback: jeśli vocation=0 ale level>8, to postać ma promocję
        if vocation == 0 and level >= 20 then
            local name = player:getName()
            vocation = getVocationId(name)
            if vocation > 0 then
            end
        end
        ...
        LoadedPlayer:setVocation(vocation)
    end
