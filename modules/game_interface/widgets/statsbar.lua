...
function StatsBar.OnGameStart()
    ...
    StatsBar.reloadCurrentTab()
    modules.game_healthcircle.setStatsBarOption()
    scheduleEvent(StatsBar.showMonkStats, 500)
end
...
function StatsBar.showMonkStats()
    local player = g_game.getLocalPlayer()
    if not player then return end
    local vocation = player:getVocation()
    local isMonk = VocationsClient and (vocation == VocationsClient.Monk or vocation == VocationsClient.ExhaltedMonk)
