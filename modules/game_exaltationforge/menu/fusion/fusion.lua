...
function Fusion:init()
    self.data = nil
    -- Inicializar valores default para quando abrir pelo preview (ícone)
    -- Esses valores serão atualizados quando o servidor enviar os dados reais
    if not self.dustNormalFusion then
        self.dustNormalFusion = 100  -- Valor padrão de dust necessário para fusion
    end
    if not self.dustConvergenceFusion then
        self.dustConvergenceFusion = 200  -- Valor padrão para convergence fusion
    end
    if not self.baseChance then
        self.baseChance = 50  -- Chance base de sucesso padrão (50%)
    end
    if not self.improvedChance then
        self.improvedChance = 75  -- Chance melhorada padrão (75%)
    end
    if not self.convergenceFusionCost then
        self.convergenceFusionCost = 0
    end
