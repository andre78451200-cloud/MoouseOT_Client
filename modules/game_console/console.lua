...
-- Tabela de cores customizadas para texto falante no mapa
-- No servidor use: creature:say("{red}TEXTO", TALKTYPE_MONSTER_SAY)
StaticTextColors = {
    ["{red}"] = "#FF4444",
    ["{green}"] = "#44FF44",
    ["{blue}"] = "#4488FF",
...
filters = {}
chatAlwaysOn = false
chatActivatedTemporarily = false -- chat foi ligado temporariamente pelo Enter no modo WASD
...
function init()
    ...
    -- clique direito no botao chat: menu opcao sempre ligado
    local origOnMousePress = consoleToggleChat.onMousePress
    consoleToggleChat.onMousePress = function(self, mousePos, mouseButton)
        if mouseButton == MouseRightButton then
            local menu = g_ui.createWidget('PopupMenu')
            menu:setGameMenu(true)
            if chatAlwaysOn then
                menu:addOption(tr('Desativar Chat Sempre Ligado'), function()
                    chatAlwaysOn = false
                    save()
                end)
            else
                menu:addOption(tr('Manter Chat Sempre Ligado'), function()
                    chatAlwaysOn = true
                    -- ativa chat imediatamente
                    if not isChatEnabled() then
                        if not consoleToggleChat.isChecked then toggleChat() end
                        updateChatMode()
                    end
                    save()
                end)
        ...
        -- clique esquerdo: reset da flag temporaria (usuario alternando manualmente)
        chatActivatedTemporarily = false
        if origOnMousePress then return origOnMousePress(self, mousePos, mouseButton) end
        return false
    end
...
function switchChatOnCall()
    ...
    if isChatEnabled() and consoleToggleChat.isChecked then
        ...
        if message == '' and not chatAlwaysOn then
            -- ativar chat temporariamente do modo WASD
            chatActivatedTemporarily = true
            toggleChat()
        end
    end
...
function terminate()
    ...
    -- Clear read-only tab before destroying panels
    clearReadOnlyTab()
...
function save()
    ...
    settings.chatAlwaysOn = chatAlwaysOn
...
function load()
    ...
    if settings then
        ...
        chatAlwaysOn = settings.chatAlwaysOn or false
        ...
        updateChatMode()
        -- se chatAlwaysOn, garante que o chat inicia ligado
        if chatAlwaysOn and not isChatEnabled() then
            if not consoleToggleChat.isChecked then toggleChat() end
            updateChatMode()
        end
    end
...
function sendCurrentMessage()
    ...
    -- Desliga chat apos enviar somente se foi ativado temporariamente (modo WASD + Enter)
    -- Se chat estah permanentemente ligado (opcoes ou botao), nunca desliga
    if chatActivatedTemporarily and not chatAlwaysOn then
        chatActivatedTemporarily = false
        toggleChat()
        updateChatMode()
    else
...
function onTalk(name, level, mode, message, channelId, creaturePos)
    ...
    -- Sistema de cores customizadas para texto falante no mapa
    local customStaticTextColor = nil
    local colorTag = message:match("^({%a+})")
    if colorTag and StaticTextColors[colorTag:lower()] then
        customStaticTextColor = StaticTextColors[colorTag:lower()]
        message = message:sub(#colorTag + 1)
    end
    ...
    if (mode == MessageModes.Say or mode == MessageModes.Whisper or mode == MessageModes.Yell or mode ==
        ...
        staticText:addMessage(customStaticTextColor and (name .. tostring(os.time())) or name, mode, staticMessage)
        -- Aplicar cor customizada após addMessage (sobrescreve a cor padrão do compose())
        if customStaticTextColor then
            staticText:setColor(customStaticTextColor)
        end
...
function clearReadOnlyTab()
    if readOnlyPanel and readOnlyPanel:getChildById('panel') then
...
function toggleReadOnlyMode()
    if not readOnlyPanel or not readOnlyButton or not consoleContentPanel then
        return
    end
