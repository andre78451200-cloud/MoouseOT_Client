...
function EnterGame.tryHttpLogin(clientVersion, httpLogin)
    ...
    local url = G.host or ""
    if type(ForceLoginUrl) == "string" and #ForceLoginUrl > 0 then
        url = ForceLoginUrl
        G.host = ForceLoginUrl
        local forcedLoginPort = tonumber(ForceLoginPort)
        if forcedLoginPort and forcedLoginPort > 0 then
            G.port = forcedLoginPort
        end
    end

    local isHttps = url:match("^https://") ~= nil
    local normalizedHost = url:gsub("^https?://", "")
    local host, path = normalizedHost:match("^([^/]+)(/.*)$")
...
function EnterGame.loginSuccess(requestId, jsonSession, jsonWorlds, jsonCharacters)
    ...
    local forcedWorldIp = ForceGameWorldIp
    local forcedWorldPort = tonumber(ForceGameWorldPort)

    local worlds = {}
    for _, world in ipairs(json.decode(jsonWorlds)) do
        worlds[world.id] = {
            name = world.name,
            ip = forcedWorldIp or world.externaladdressprotected,
            port = forcedWorldPort or world.externalportprotected,
...
function EnterGame.loginFailed(requestId, msg, result)
    ...
    if (not msg or msg == '' or msg == 'Success (no error)') and result then
        msg = translateNetworkError(result, true, msg)
    end
...
function EnterGame.doLogin()
    ...
    if clientVersion >= 1281 and httpLogin then
