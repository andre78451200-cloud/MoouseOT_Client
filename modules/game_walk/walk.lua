...
local function canChangeFloor(pos, deltaZ)
    ...
    if deltaZ > 0 then
        -- Going DOWN
        return toTile:isWalkable() and toTile:hasElevation(3)
    end
    -- deltaZ < 0
...
local function addWalkEvent(dir, delay)
    -- Always cancel previous walk event to prevent orphaned events
    -- that bypass the inter-step delay from onWalkFinish.
    if walkEvent then
        removeEvent(walkEvent)
        walkEvent = nil
    end
