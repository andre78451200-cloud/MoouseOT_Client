...
function online()
    if g_game.getFeature(GameUnjustifiedPoints) then
        if not unjustifiedPointsButton then
        ...
        -- Sync button state with window state after setupOnStart
        unjustifiedPointsButton:setOn(unjustifiedPointsWindow:isVisible())
    end
