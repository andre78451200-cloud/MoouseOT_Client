...
function Cyclopedia.CreateCreatureItems(data)
    ...
    for index, _ in pairs(data) do
        ...
        for itemIndex, itemData in ipairs(data[index]) do
            if itemIndex > 15 then break end
            ...
            if not itemWidget then break end
...
-- Cache local de todas as races para busca case-insensitive
local bestiaryRaceCache = nil

local function buildBestiaryRaceCache()
    if bestiaryRaceCache then return end
    -- getRacesByName("") retorna TODAS as races do staticdata
    local allRaces = g_things.getRacesByName("")
    bestiaryRaceCache = {}
    for _, race in ipairs(allRaces) do
        if race.name and race.name ~= "" then
            table.insert(bestiaryRaceCache, {
                raceId = race.raceId,
                name = race.name,
                nameLower = race.name:lower()
            })
        end
    end
end
...
function Cyclopedia.BestiarySearch()
    ...
    -- Constroi cache na primeira busca
    buildBestiaryRaceCache()

    -- Busca case-insensitive no Lua (nao depende do C++)
    local searchLower = text:lower()
    local matches = {}

    for _, race in ipairs(bestiaryRaceCache) do
        if race.nameLower:find(searchLower, 1, true) then
            table.insert(matches, race)
        end
    end

    if #matches == 1 then
        -- Resultado unico: abre detalhes da criatura diretamente
        g_game.requestBestiarySearch(matches[1].raceId)
        Cyclopedia.ShowBestiaryCreature()
    elseif #matches > 0 then
        -- Multiplos resultados: mostra lista de resultados
        local searchData = {}
        ...
        Cyclopedia.loadBestiarySearchCreatures(searchData)
    end
