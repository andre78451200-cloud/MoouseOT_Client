...
local function attachEffectIfValid(UICreature, value)
    ...
    if checkPresetsValidity({value}) then
        if creature then
            local effect = g_attachedEffects.getById(value)
            if effect then
...
local function showSelectionList(data, tempValue, tempField, onSelectCallback)
    ...
    if data and #data > 0 then
        for _, itemData in ipairs(data) do
            ...
            local Category = modules.game_attachedeffects and modules.game_attachedeffects.getCategory(itemData[1]) or 0
            if Category == 1 then
                button.outfit:setOutfit({
                    type = modules.game_attachedeffects and modules.game_attachedeffects.thingId(itemData[1]) or 0
                })
            elseif Category == 2 then
                button.outfit:setOutfit(previewCreature:getCreature():getOutfit())
                local effect = g_attachedEffects.getById(itemData[1])
                if effect then
                    button.outfit:getCreature():attachEffect(effect)
                end
            elseif Category == 5 then
                button.outfit:setImageSource(modules.game_attachedeffects and modules.game_attachedeffects.getTexture(itemData[1]) or "")
            end

            button.name:setText(modules.game_attachedeffects and modules.game_attachedeffects.getName(itemData[1]) or "None")
...
function showHealthBars()
    ...
    if ServerData.healthBars and #ServerData.healthBars > 0 then
        for _, barData in ipairs(ServerData.healthBars) do
            ...
            local Category = modules.game_attachedeffects and modules.game_attachedeffects.getCategory(barData) or 0
            if Category == 5 then
                button.outfit:setImageSource(modules.game_attachedeffects and modules.game_attachedeffects.getTexture(barData) or "")
                button.outfit:setWidth(64)
                button.outfit:setHeight(32)
            else
                button.outfit:setOutfit(previewCreature:getCreature():getOutfit())
                local effect = g_attachedEffects.getById(barData)
                if effect then
                    button.outfit:getCreature():attachEffect(effect)
                end
            end
...
function onPresetSelect(list, focusedChild, unfocusedChild, reason)
    ...
    if focusedChild then
        ...
        updateAppearanceText("preset", preset.title)
        updateAppearanceText("aura", modules.game_attachedeffects and modules.game_attachedeffects.getName(preset.auras) or "None")
        updateAppearanceText("wings", modules.game_attachedeffects and modules.game_attachedeffects.getName(preset.wings) or "None")
        updateAppearanceText("shader", preset.shaders or "Outfit - Default")
        updateAppearanceText("effects", modules.game_attachedeffects and modules.game_attachedeffects.getName(preset.effects) or "None")
...
function onAuraSelect(list, focusedChild, unfocusedChild, reason)
    ...
    if focusedChild then
        ...
        if checkPresetsValidity({auraType}) then
            local effect = g_attachedEffects.getById(auraType)
            if effect then
                previewCreature:getCreature():attachEffect(effect)
            end
            lastSelectAura = auraType
            tempOutfit.auras = auraType
            updatePreview()
            deselectPreset()
            updateAppearanceText("aura", modules.game_attachedeffects and modules.game_attachedeffects.getName(auraType) or "None")
        else
            lastSelectAura = "None"
            tempOutfit.auras = 0
            updateAppearanceText("aura", "None")
...
function onWingsSelect(list, focusedChild, unfocusedChild, reason)
    ...
    if focusedChild then
        ...
        if checkPresetsValidity({wingsType}) then
            local effect = g_attachedEffects.getById(wingsType)
            if effect then
                previewCreature:getCreature():attachEffect(effect)
            end
            ...
            updatePreview()
            deselectPreset()
            updateAppearanceText("wings", modules.game_attachedeffects and modules.game_attachedeffects.getName(wingsType) or "None")
        else
            lastSelectWings = "None"
            tempOutfit.wings = 0
            updateAppearanceText("wings", "None")
...
function onHealthBarSelect(list, focusedChild, unfocusedChild, reason)
    ...
    if focusedChild then
        ...
        updatePreview()
        if barType > 0 then
            local effect = g_attachedEffects.getById(barType)
            if effect then
                previewCreature:getCreature():attachEffect(effect)
            end
        end
...
function onEffectBarSelect(list, focusedChild, unfocusedChild, reason)
    ...
    if focusedChild then
        ...
        if checkPresetsValidity({effect_id}) then
            local effect = g_attachedEffects.getById(effect_id)
            if effect then
                previewCreature:getCreature():attachEffect(effect)
            end
            ...
            updatePreview()
            deselectPreset()
            updateAppearanceText("effects", modules.game_attachedeffects and modules.game_attachedeffects.getName(effect_id) or "None")
        else
            updateAppearanceText("effects", "None")
            lastSelectEffects = "None"
