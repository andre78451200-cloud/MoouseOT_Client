...
function terminate()
    ...
    if spellDragWidget then
        spellDragWidget:destroy()
        spellDragWidget = nil
    end
...
function createSpellDragWidget(spellName, iconId, mousePos)
    if spellDragWidget then
        spellDragWidget:destroy()
    end
    
    spellDragWidget = g_ui.createWidget('UIWidget', g_ui.getRootWidget())
    spellDragWidget:setId('spellDragWidget')
    spellDragWidget:setSize({width = 32, height = 32})
    spellDragWidget:setImageSource(SpelllistSettings[SpelllistProfile].iconFile)
    spellDragWidget:setImageClip(Spells.getImageClip(iconId, SpelllistProfile))
    spellDragWidget:setPhantom(true)
    spellDragWidget:setOpacity(0.8)
    spellDragWidget:raise()
    spellDragWidget:setPosition({x = mousePos.x - 16, y = mousePos.y - 16})
    spellDragWidget.currentDragSpell = spellName
    
    g_mouse.pushCursor('target')
end

function destroySpellDragWidget()
    if spellDragWidget then
        spellDragWidget:destroy()
        spellDragWidget = nil
    end
...
function updateSpellDragWidgetPosition(mousePos)
    if spellDragWidget then
        spellDragWidget:setPosition({x = mousePos.x - 16, y = mousePos.y - 16})
    end
end
...
function initializeSpelllist()
    for i = 1, #SpelllistSettings[SpelllistProfile].spellOrder do
        ...
        -- Use clientId if available (higher priority)
        if info.clientId then
            iconId = info.clientId
        end

        -- Fallback: use a default icon if not found (avoids error spam in console)
        if not iconId then
            -- perror('Spell icon \'' .. tostring(info.icon) .. '\' not found.')
            iconId = 1 -- Default to first icon
        end
        
        -- Store iconId for drag widget
        tmpLabel.iconId = iconId
        ...
        tmpLabel.onDragEnter = function(self, mousePos)
            currentDraggedSpell = self.spellName
            self:setOpacity(0.5)
            createSpellDragWidget(self.spellName, self.iconId, mousePos)
            return true
        end
        ...
        tmpLabel.onDragLeave = function(self, droppedWidget, mousePos)
            ...
            if currentDraggedSpell and mousePos then
                -- Use global function from game_actionbar module
                if _G.dropSpellOnActionBar then
                    _G.dropSpellOnActionBar(currentDraggedSpell, mousePos)
                end
            end
            
            currentDraggedSpell = nil
            destroySpellDragWidget()
            return true
        end
        
        tmpLabel.onDragMove = function(self, mousePos, mouseMoved)
            updateSpellDragWidgetPosition(mousePos)
            return true
        end
    end
...
function updateSpelllist()
    for i = 1, #SpelllistSettings[SpelllistProfile].spellOrder do
        ...
        if filters.vocationId == FILTER_VOCATION_ANY then
            vocationMatch = true
        elseif filters.vocationId == FILTER_VOCATION_MONK then
            -- Monk uses vocations 9 and 10
            vocationMatch = table.find(info.vocations, 9) or table.find(info.vocations, 10)
        else
            -- Other vocations: base (1-4) and promoted (+4 = 5-8)
            vocationMatch = table.find(info.vocations, filters.vocationId) or table.find(info.vocations, filters.vocationId + 4)
        end
