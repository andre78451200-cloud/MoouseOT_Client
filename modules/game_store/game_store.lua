...
function onParseStoreGetPurchaseStatus(purchaseStatus)
    ...
    -- fallback: some OTC versions don't auto-set Lua fields for child widgets
    if not messageBox.buttonAnimation then
        messageBox.buttonAnimation = messageBox:recursiveGetChildById('buttonAnimation')
    end
    if not messageBox.Box then
        messageBox.Box = messageBox:recursiveGetChildById('Box')
    end
    if messageBox.Box then
        messageBox.Box:setText(purchaseStatus)
    end
    if not messageBox.buttonAnimation then return end
    if messageBox.buttonAnimation.animation then
...
function transferPoints()
    ...
    if playerBalance == 0 then
        playerBalance = coinsBalance2 -- fallback: use normal coins
    end

    g_logger.info(string.format("[transferPoints] resource=%d label_transfer=%d label_normal=%d final=%d",
        g_game.getLocalPlayer():getResourceBalance(ResourceTypes.COIN_TRANSFERRABLE),
        coinsBalance1, coinsBalance2, playerBalance))
    ...
    transferPointsWindow.amountBar:setMaximum(maxStep > 0 and maxStep or playerBalance)
    ...
    transferPointsWindow.amountBar.onValueChange = function(scrollbar, value)
        transferPointsWindow.amount:setText(formatNumberWithCommas(value))
    end
    ...
    transferPointsWindow.buttonOk.onClick = function()
        ...
        -- fallback: read from the displayed label if slider returns 0
        if amount < 1 then
            local txt = transferPointsWindow.amount:getText():gsub("[^%d]", "")
            amount = tonumber(txt) or 0
        end
        g_logger.info(string.format("[transferPoints gift] receipient='%s' amount=%d", receipient, amount))
