...
function onExecuteAction(button, isPress)
    ...
    if action == UseTypes["Equip"] and button.item then
        ...
        if not player or player:getInventoryCount(button.cache.itemId, tier) == 0 then
            return
        end
...
function updateButtonState(button)
    ...
    if not player.hasEquippedItemId then
        return
    end
...
function updateButton(button)
    ...
    if useAction then
        ...
        if spellData then
            ...
            if spellData.vocations and player and not table.contains(spellData.vocations, translateVocation(player:getVocation())) then
