...
function init()
    ...
    -- Expose function globally for cross-module spell drag & drop
    _G.dropSpellOnActionBar = dropSpellOnActionBar
...
function assignSpellToSlot(slotId, spellName)
...
-- Global function for dropping spells from spell list
function dropSpellOnActionBar(spellName, mousePos)
    if not spellName or not mousePos or not actionBarPanel then return false end
    
    local children = actionBarPanel:getChildren()
    for _, slot in ipairs(children) do
        local pos = slot:getPosition()
        local size = slot:getSize()
        if mousePos.x >= pos.x and mousePos.x <= pos.x + size.width and
           mousePos.y >= pos.y and mousePos.y <= pos.y + size.height then
            return assignSpellToSlot(slot:getId(), spellName)
        end
    end
    return false
end
    ...
    for i = 1, maxSlots do
        ...
        if not isLocked then
            g_mouse.bindOnDrop(slot, function()
                -- Check if a spell is being dragged from spell list
                if currentDraggedSpell then
                    assignSpellToSlot('slot' .. i, currentDraggedSpell)
                    currentDraggedSpell = nil
                    -- Destroy drag widget if exists
                    if destroySpellDragWidget then
            ...
            -- Handle onDrop event for spell drops
            slot.onDrop = function(self, widget, mousePos)
                -- Check for spell drag widget
                if widget and widget.currentDragSpell then
                    assignSpellToSlot('slot' .. i, widget.currentDragSpell)
                    currentDraggedSpell = nil
                    if destroySpellDragWidget then
                        destroySpellDragWidget()
                    end
                ...
                -- Check global drag spell variable
                if currentDraggedSpell then
                    assignSpellToSlot('slot' .. i, currentDraggedSpell)
                    currentDraggedSpell = nil
                    if destroySpellDragWidget then
