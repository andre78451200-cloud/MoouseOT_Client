...
local imageDir = '/images/game/analyzer/resists/monster-icon-%s-resist'
...
function onImpactExtra(mousePosition)
  if cancelNextRelease then
	...
	local damageTypeVisible = ImpactAnalyser.window.contentsPanel.damageTypeLabel:isVisible()
	...
	menu:addCheckBox(tr('DPS gauge'), dpsGaugeVisible, function() ImpactAnalyser:setDPSGauge(not dpsGaugeVisible, true) end)
	menu:addCheckBox(tr('DPS graph'), dpsGraphVisible, function() ImpactAnalyser:setDPSGraph(not dpsGraphVisible, true) end)
	menu:addSeparator()
	menu:addCheckBox(tr('Damage Types'), damageTypeVisible, function() ImpactAnalyser:setDamageType(not damageTypeVisible, true) end)
	menu:addSeparator()
	menu:addOption(tr('Set HPS target'), function() ImpactAnalyser:openTargetConfig(false) end)
	menu:addCheckBox(tr('HPS gauge'), hpsGaugeVisible, function() ImpactAnalyser:setHPSGauge(not hpsGaugeVisible, true) end)
	menu:addCheckBox(tr('HPS graph'), hpsGraphVisible, function() ImpactAnalyser:setHPSGraph(not hpsGraphVisible, true) end)
