...
function HuntingAnalyser:updateWindow(ignoreVisible)
	...
	-- Raw XP/H: calcula baseado no rawXPGain acumulado / tempo decorrido
	local elapsed = math.floor((g_clock.millis() - HuntingAnalyser.launchTime) / 1000)
	if rawExperience > 0 and elapsed > 0 then
		HuntingAnalyser.rawXpHour = math.floor((rawExperience / elapsed) * 3600)
	else
		HuntingAnalyser.rawXpHour = 0
	end
