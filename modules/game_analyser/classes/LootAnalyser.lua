...
function LootAnalyser:updateBasePriceFromLootedItems(itemId, newPriceValue)
	...
	if itemInfo then
		if not newPriceValue then
			-- Use Analyser:getSalePrice for consistency
			newPriceValue = Analyser:getSalePrice(nil, itemId)
		end
...
function LootAnalyser:updateGraphics()
	...
	-- Always calculate per hour based on actual elapsed time
...
function LootAnalyser:addLootedItems(item, name)
	...
	-- Use Analyser:getSalePrice for consistency with HuntingAnalyser
	if itemInfo.basePrice == 0 then
		itemInfo.basePrice = Analyser:getSalePrice(item, itemId)
	end
