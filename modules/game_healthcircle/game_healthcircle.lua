...
function init()
    -- Initialize module references inside init()
...
function initOnHpAndMpChange()
    connect(LocalPlayer, {
        ...
        -- onManaShieldChange = whenManaShieldChange, -- Disabled: event not available in this client
...
local function updateManaShieldDisplay()
    -- ManaShield display disabled - methods getMaxManaShield/getManaShield not available in this client version
    -- If you need ManaShield display, you need to compile the client with these methods
    if not manaShieldCircle or not manaShieldCircleFront or not manaCircle or not manaCircleFront then
        return
    end

    -- Always hide mana shield circles since methods don't exist
    manaShieldCircle:setVisible(false)
    manaShieldCircleFront:setVisible(false)
...
function setPlayerValues()
    ...
    -- @ MONK: Check if player is Monk on login
    checkMonkVocation()
    -- @
end
